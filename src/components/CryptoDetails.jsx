...
const CryptoDetails = () => {
  ...
  const {data, isFetching}=useGetCryptoDetailsQuery({coinId, params: {
    referenceCurrencyUuid: coinId,
    timePeriod: '24h'
  },})
   const {data:coinHistory}=useGetCryptosHistoryQuery(coinId,setTimePeriod)
  const cryptoDetails = data?.data?.coin
  console.log('useGetCryptosHistoryQuery', coinHistory);
  console.log('useGetCryptoDetailsQuery', data);

  if(isFetching) return '<Loader/>'
  const time = ['3h', '24h', '7d', '30d', '1y', '3m', '3y', '7y'];
  ...
  return (
   ...
   <Col className='coin-detail-container'>
    ...
    <Select defaultValue='7d' className='select-timeperiod' placeholder='Select Time Period' onChange={(value)=>setTimePeriod}>
